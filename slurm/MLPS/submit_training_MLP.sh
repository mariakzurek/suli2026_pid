...
WORKER_SCRIPT="${SCRIPT_DIR}/_training_MLP_job.sh"
...
if [ -z "${FEATURES_FILE}" ]; then
    ...
    echo "" >&2
    echo "Example:" >&2
    echo "  ./slurm/submit_training_MLP.sh \\" >&2
...
SBATCH_CMD=(
    ...
    --output="${FARM_LOG_DIR}/training_MLP_%j.out"
    --error="${FARM_LOG_DIR}/training_MLP_%j.err"
    --export="${EXPORT_VARS}"
    "${WORKER_SCRIPT}"
)
