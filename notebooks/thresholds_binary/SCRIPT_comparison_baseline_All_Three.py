...
mod2, mod_df2 = au.load_model_and_data("/work/clas12/CooperBe/MLStuff/tier2All/model_v02/model.joblib", df_val)
allPlots=[]
...
csvFile2 = "../../figures/binary_MLP/comparisons/optimized_thresholds_BDT.csv"
...
if Path(csvFile2).exists():
    print(f"Loading optimized thresholds from {csvFile}")
    results_df2 = pd.read_csv(csvFile2)
else:
    print("Optimized threshold file not found. Running optimization...")
    results_df2 = au.optimizeFOM(
        mod_df2,
        tBinEdges,
        pBinEdges,
        outputCSV=csvFile2,
...
df_test2 = au.apply_model_to_df(mod2, df_test, feature_names)


bdtMask=au.apply_optimized_bdt_cut(df_test, threshold_df=results_df)
bdtMask2=au.apply_optimized_bdt_cut(df_test, threshold_df=results_df2)
...
bdtMask2 = au.apply_optimized_bdt_cut(
    df_test,
    threshold_df=results_df2
)
...
for i in range(len(pBinEdges)-1):
    ...
    bdt_bin_mask2 = bdt_bin_mask2[theta_bin_mask]
    ...
    bdt_eff2, bdt_err2 = au.compute_efficiency(
        df_pbin,
        cut=bdt_bin_mask2
    )
    ...
    bdt_vals2.append(bdt_eff2)
    bdt_errs2.append(bdt_err2)

    chi_vals.append(chi_eff)
    chi_errs.append(chi_err)


bdt_vals2 = np.array(bdt_vals2)
bdt_errs2 = np.array(bdt_errs2)
...
outpath = Path(
    "../../figures/binary_MLP/comparisons/efficiency_All_Three.png"
)
...
for i in range(len(pBinEdges)-1):
    ...
    bdt_subset2 = df_pbin[bdt_bin_mask2]
    ...
    bdt_cont2, bdt_err2 = au.compute_contamination(
        bdt_subset2
    )
...
# Save
outpath = Path(
    "../../figures/binary_MLP/comparisons/contamination_All_Three.png"
)
