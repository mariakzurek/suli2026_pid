...
def compute_contamination_vs_momentum(
    ...
    for _, row in result_df.iterrows():
        ...
        rect = Rectangle(
            (
                row["p_lo"],
                row["contamination"] - row["syst_err"]
            ),
            row["p_hi"] - row["p_lo"],
            2 * row["syst_err"],
            color="blue",
...
def plot_uniform_systematics_vs_momentum(
    master,
    theta_range,
    p_range,
    num_bins=10,
    output_png="uniform_systematics.png",
    output_csv="uniform_systematics.csv",
    title="Uniform systematic uncertainty vs momentum",
):
    """
    Rebin systematic uncertainties into uniform momentum bins.

    The original master table is left untouched. For each new uniform
    momentum bin:
      - find all original systematic bins overlapping that bin
      - average each systematic source separately
      - recompute quadrature from the averaged sources

    Parameters
    ----------
    master : pd.DataFrame
        Output from make_systematic_quadrature_table()
    ...
    if subset.empty:
        raise ValueError(
            "No systematic bins found in requested theta range"
        )


    # ---------------------------------------------
    # Identify systematic sources
    # ---------------------------------------------
    ...
    for i in range(num_bins):
        ...
        # -----------------------------------------
        # Average each systematic source
        # -----------------------------------------

        for col in sys_cols:

            if len(overlap) > 0:
                row[col] = overlap[col].mean()
            else:
                row[col] = np.nan


        # -----------------------------------------
        # Recalculate quadrature
        # -----------------------------------------
...
# per-bin CSVs
# ---------------------------------------------------------------
uniform_sys_MC = plot_uniform_systematics_vs_momentum(
    master_Mc,
    theta_range=(5,15),
    p_range=(0.5,5),
    num_bins=10,
    output_csv="pngs/uniform_systematics_MC.csv",
    output_png="pngs/uniform_systematics_MC.png",
    title="Uniform Systematic Uncertainty, MC"
)


uniform_sys_RICH = plot_uniform_systematics_vs_momentum(
    master_RICH,
    theta_range=(0,20),
    p_range=(2.75,5),
    num_bins=10,
    output_csv="pngs/uniform_systematics_RICH.csv",
    output_png="pngs/uniform_systematics_RICH.png",
    title="Uniform Systematic Uncertainty, RICH"
)


uniform_sys_MX = plot_uniform_systematics_vs_momentum(
    master_Mx,
    theta_range=(5,20),
    p_range=(2.75,5),
    num_bins=10,
    output_csv="pngs/uniform_systematics_MX.csv",
    output_png="pngs/uniform_systematics_MX.png",
    title="Uniform Systematic Uncertainty, MX"
)