...
Reconcile the π-mis-ID number with the RICH-derived kaon-contamination number from Thread 1 in the (p, θ) region where both are populated. The two quantities are not the same — the RICH number is `C^{π→K}` on the K-selection channel, the ratio-method number is the π-mis-ID-as-K rate on a pion-truth sample extracted from within the EB-K⁺ data — but under the assumption that the classifier's π-mis-ID rate is a property of the classifier rather than the sample composition, they constrain each other. Consistency under that assumption is the two-way cross-check the report leans on. Disagreement is a finding, not a failure, and joins the Thread-1 diagnosis rather than being averaged past.
